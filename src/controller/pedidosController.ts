  ...
  try {
    ...
    for (const pedidoPendiente of pedidosPendientes) {
      if (stockDisponibleActual >= pedidoPendiente.cantidad) {
        ...
        console.log(`🔴 Pedido ${pedidoPendiente.idPedido} no puede ser reservado - Stock insuficiente (Necesita: ${pedidoPendiente.cantidad}, Disponible: ${stockDisponibleActual}) - Estado cambiado a PENDIENTE`);
      }
    }
  } catch (error) {
  ...
  try {
    ...
    const pedidosFormateados = pedidos.map((pedido) => {
      ...
      return {
        ...
        valor: `$${primerProducto && !isNaN(primerProducto.valor)
          ? primerProducto.valor.toFixed(2)
          : "0.00"
          }`,
        ...
        productos: pedido.productos.map((prod: any) => ({
          ...
          comentario_producto: prod.comentario_producto,
          modelo: prod.modelo,
...
export const cambiarEstadoAEntregado = async (
  ...
  try {
    ...
    for (const producto of pedido.productos) {
      ...
      console.log(
        `   - ${cantidadRealEntregada} unidades restadas del stock.`
      );
      console.log(
        `   - ${cantidadRealEntregada} unidades restadas del reservado.`
      );
      console.log(
        `   - Pedido eliminado del array pedidos.`
      );
    }
    ...
    res.status(200).json({
      message:
        "Productos entregados, stock actualizado correctamente.",
...
export const updatePedido = async (
  ...
  try {
    ...
    if (updates.productos && Array.isArray(updates.productos)) {
      ...
      console.log("📋 Productos anteriores:", productosAnteriores.map((p: any) => ({
        idStock: p.idStock,
        idModelo: p.idModelo,
        cantidad: p.cantidad,
        estado_stock: p.estado_stock
      })));
      console.log("📋 Productos nuevos:", productosNuevos.map((p: any) => ({
        idStock: p.idStock,
        idModelo: p.idModelo,
        cantidad: p.cantidad,
        estado_stock: p.estado_stock
      ...
      const productosEliminados = productosAnteriores.filter((prodAnterior: any) =>
        !productosNuevos.some((prodNuevo: any) =>
          prodNuevo.idStock.toString() === prodAnterior.idStock.toString()
        )
      );

      if (productosEliminados.length > 0) {
        console.log("🗑️ === PRODUCTOS ELIMINADOS ===");
        console.log("📋 Productos eliminados:", productosEliminados.map((p: any) => ({
          idStock: p.idStock,
          idModelo: p.idModelo,
          cantidad: p.cantidad,
          estado_stock: p.estado_stock
        ...
        for (const productoEliminado of productosEliminados) {
          ...
          const pedidoEnStock = stock.pedidos.find((p: any) =>
            p.idPedido.toString() === (pedidoExistente._id as any).toString()
          );
          ...
          if (pedidoEnStock) {
            ...
            if (pedidoEnStock.estado === "reservado") {
              ...
              console.log(`✅ Stock reservado decrementado para ${productoEliminado.idStock}: -${pedidoEnStock.cantidad} unidades`);
              ...
              console.log(`✅ Stock pendiente decrementado para ${productoEliminado.idStock}: -${pedidoEnStock.cantidad} unidades`);
              console.log(`✅ Pedido eliminado del array pedidos`);
            }
          } else {
      ...
      const productosNuevosAgregados = productosNuevos.filter((prodNuevo: any) =>
        !productosAnteriores.some((prodAnterior: any) =>
          prodAnterior.idStock.toString() === prodNuevo.idStock.toString()
        )
      );

      if (productosNuevosAgregados.length > 0) {
        console.log("🆕 === PRODUCTOS NUEVOS ===");
        console.log("📋 Productos nuevos:", productosNuevosAgregados.map((p: any) => ({
          idStock: p.idStock,
          idModelo: p.idModelo,
          cantidad: p.cantidad,
          estado_stock: p.estado_stock
        ...
        for (const productoNuevo of productosNuevosAgregados) {
          ...
          if (stockDisponible >= cantidadRealNecesaria) {
            ...
            console.log(`🔴 Producto nuevo PENDIENTE para ${productoNuevo.idStock}: ${cantidadRealNecesaria} unidades (Stock insuficiente)`);
          }
        }
      }
      ...
      for (const productoNuevo of productosNuevos) {
        ...
        if (productoAnterior && productoNuevo.cantidad !== productoAnterior.cantidad) {
          ...
          console.log("🔍 Pedido en stock:", pedidoEnStock ? {
            idPedido: pedidoEnStock.idPedido,
            cantidad: pedidoEnStock.cantidad,
            estado: pedidoEnStock.estado
          } : "No encontrado");
          ...
          if (pedidoEnStock) {
            ...
            if (pedidoEnStock.estado === "reservado") {
              ...
              console.log(`✅ Pedido cambiado de RESERVADO a PENDIENTE para ${productoNuevo.idStock}: ${pedidoEnStock.cantidad} → ${cantidadRealNueva} unidades`);
            } else if (pedidoEnStock.estado === "pendiente") {
              ...
              console.log(`✅ Cantidad pendiente actualizada para ${productoNuevo.idStock}: ${cantidadRealDiferencia} unidades`);
            }
          } else {
            ...
            console.log(`✅ Nuevo pedido agregado como PENDIENTE para ${productoNuevo.idStock}: ${cantidadRealNueva} unidades`);
          }
          ...
          const pedidoEnStockActualizado = stockActualizado?.pedidos.find((p: any) =>
            p.idPedido.toString() === (pedidoExistente._id as any).toString()
          );
    ...
    if (updates.productos && Array.isArray(updates.productos)) {
      ...
      if (pedidoActualizadoTemp) {
        updatesFinal.productos = await Promise.all(updates.productos.map(async (prod: any, index: number) => {
          const productoActualizado = pedidoActualizadoTemp.productos[index];

          // ✅ Verificar si es un producto nuevo
          const esProductoNuevo = !pedidoExistente.productos.some((p: any) => p.idStock.toString() === prod.idStock.toString());
...
export const deletePedido = async (
  ...
  try {
    ...
    for (const producto of pedidoAEliminar.productos) {
      ...
      // ✅ Buscar el pedido en el array pedidos del stock
      const pedidoEnStock = stock.pedidos.find((p: any) =>
        p.idPedido.toString() === (pedidoAEliminar._id as any).toString()
      );
...
export const añadirComentario = async (
  ...
  try {
    ...
    if (tipo === "cliente") {
      ...
      console.log(`✅ Comentario de producto ${comentarioAnterior ? 'reemplazado' : 'añadido'} correctamente al índice ${indiceProducto}`);
      if (comentarioAnterior) {
        console.log(`📝 Comentario anterior: "${comentarioAnterior}"`);
      }
      console.log(`📝 Nuevo comentario: "${comentario}"`);
